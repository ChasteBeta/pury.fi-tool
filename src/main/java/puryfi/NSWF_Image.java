...
public class NSWF_Image {
    ...
    public void pixelate(BufferedImage inputImage, Rectangle boundingbox) {
        ...
        int endY = setCoordinateWithinBounds(boundingbox.y + boundingbox.height, inputImage.getHeight());
        int startX = setCoordinateWithinBounds(boundingbox.x, inputImage.getWidth());
        int endX = setCoordinateWithinBounds(boundingbox.x + boundingbox.width, inputImage.getWidth());

        // Loop through every PIX_SIZE pixels, in both x and y directions
        for (int y = startY; y < endY; y += PIX_SIZE) {
            for (int x = startX; x < endX; x += PIX_SIZE) {

                // "Paste" the pixel onto the surrounding PIX_SIZE by PIX_SIZE neighbors
                // Also make sure that our loop never goes outside the bounds of the image
                for (int yd = y; (yd < y + PIX_SIZE) && (yd < inputImage.getHeight()); yd++) {
                    for (int xd = x; (xd < x + PIX_SIZE) && (xd < inputImage.getWidth()); xd++) {
                        if (xd >= boundingbox.x && xd <= boundingbox.x + boundingbox.width
                                && yd >= boundingbox.y && yd <= boundingbox.y + boundingbox.height) {
                            inputImage.setRGB(xd, yd, inputImage.getRGB(x, y));
                        } else {
                            inputImage.setRGB(xd, yd, inputImage.getRGB(xd, yd));
                        }
    ...
    public static int setCoordinateWithinBounds(int coordinate, int max){
        if(max <= coordinate){
            coordinate = max - 1;
        }
        if(coordinate < 0){
